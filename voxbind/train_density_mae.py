"""train_density_mae.py — DDP voxel-MAE pre-training for the density encoder.

Phase 1 — Stage A (synthetic density). Generates 2Fo-Fc-like volumes on-the-fly
from voxelized ligand+pocket atoms (Gaussian blur + per-sample z-score + noise),
applies 3D block masking, and trains a `DensityMAE` to (a) inpaint the masked
density and (b) predict the 11-channel atom voxel grid.

Phase 2 (Stage B, real x-ray adapt on `xray_crops_aligned/`) deferred.
Phase 3 (load encoder into VoxBind) deferred.

Launch
------
    cd voxbind
    CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --standalone --nproc_per_node=4 \
        train_density_mae.py \
        exp_name=260520_density_mae_pretrain \
        output_dir=exps/260520_density_mae_pretrain

Notes
-----
* cfg.bsz is per-rank (matches train_ddp.py contract).
* No `wjs` sampling step — pre-training only.
* Checkpoint saves both `state_dict_ema` (full DensityMAE) AND a separate
  `encoder_state_dict_ema` for easy loading into VoxBind later.
"""
...
from voxbind.models.density_mae import (
    DensityMAE, gaussian_blur3d, make_block_mask, per_sample_zscore,
)
...
logger = logging.getLogger("train-density-mae")
    ...
    def _step(self, batch):
        ...
        with torch.cuda.stream(self.stream):
            with torch.no_grad():
                ...
                # Synthetic density: blur(atoms_sum) → z-score → +noise
                ...
                target_str = torch.cat([v_lig, v_poc], dim=1)   # (B, 11, G, G, G)
...
def compute_losses(
    ...
    """Per-voxel-averaged MSE on masked density + weighted-MSE structure target.

    Density: plain MSE on masked voxels (z-scored density isn't sparse).
    Structure: weighted MSE — voxels where target > `struct_pos_thresh` get
    `struct_pos_weight`× the weight of background voxels. Without this,
    99%+ of voxels are background and the model collapses to outputting ~0
    everywhere (the zero-MSE shortcut), so loss looks small while predicting
    no atoms at all on the sparse ligand target.

    Returns (L_dens, L_str).
    """
...
def precompute_val(loader_val, voxelizer, cfg) -> dict:
    """Cache voxelized val ligands+pockets to disk so each rank reuses them."""
...
def val_epoch(cfg, model, val_cache, device) -> dict:
    ...
    with torch.no_grad():
        for batch_idx, start in enumerate(range(0, n, cfg.bsz)):
            # Re-seed at the start of each batch so noise + mask are
            # epoch-stable (same val numbers from epoch to epoch).
            ...
            atoms_sum = v_lig.sum(dim=1, keepdim=True) + v_poc.sum(dim=1, keepdim=True)
            sigma_vox = sigma_lo + (sigma_hi - sigma_lo) * 0.5  # midpoint, deterministic
            d_clean = gaussian_blur3d(atoms_sum, sigma_vox)
            d_clean = per_sample_zscore(d_clean)
    ...
    for i, (d_in, d_clean, mask, target_str) in enumerate(prefetcher):
        ...
        if is_step:
            if grad_clip > 0:
                ...
                # to catch spikes (a regression-detection trick that costs ~0).
...
@hydra.main(config_path="configs", config_name="config_train_density_mae", version_base=None)
def main(cfg: DictConfig) -> None:
    ...
    # Resume (full cfg reload, like train_ddp.py)
    ...
    model = DensityMAE(
        ...
        n_struct_channels=cfg.model.n_struct_channels,
    ).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    if is_main:
        logger.info(f"DensityMAE has {(n_params/1e6):.02f}M parameters")
    ...
    # DDP wrap. find_unused_parameters=False — no conditional branches here.
    if world_size > 1:
    ...
    # Val cache (rank 0 writes, others wait+load)
    if is_main:
    ...
    # σ_blur converted Å → voxels (resolution = 0.25 Å/vox by default)
    ...
    if is_main:
        logger.info("start density-MAE pre-training...")
    ...
    for epoch in epochs_iter:
        ...
        if is_main:
            ...
            # Save: full model EMA + encoder-only state_dict for downstream load
            encoder_sd = {
