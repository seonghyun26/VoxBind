...
# Run from the project root: bash scripts/02_sample.sh [OPTIONS]
#
# Usage:
#   bash scripts/02_sample.sh [sigma] [split] [n_samples] [n_targets]
#
...
#
# Examples:
#   bash scripts/02_sample.sh                        # sigma=0.9, test split, 10 samples, 100 targets
#   bash scripts/02_sample.sh 0.9 val 10 100         # validation split
#   bash scripts/02_sample.sh 1.0 test 20 100
#
...
if [ ! -d "$PRETRAINED_PATH" ]; then
    ...
    echo "Run 01_train.sh first, or set the correct sigma."
    exit 1
fi
