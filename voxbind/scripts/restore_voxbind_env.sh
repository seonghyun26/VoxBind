...
# This is the FAST path (~30s extract) vs scripts/rebuild_voxbind_env.sh (~10min, needs pip/net).
...
[ -f "$BACKUP" ] || { echo "[$(ts)] NO BACKUP at $BACKUP -> run scripts/rebuild_voxbind_env.sh instead"; exit 1; }
