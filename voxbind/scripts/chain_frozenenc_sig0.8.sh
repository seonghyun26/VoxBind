...
#   setsid nohup bash scripts/chain_frozenenc_sig0.8.sh > log/chain_frozenenc_sig0.8.log 2>&1 &
set -uo pipefail
VOX=/home1/irteam/VoxBind/voxbind; B=/opt/conda/envs/voxbind/bin; LOG=$VOX/log
POSTLOG=$LOG/chain_frozenenc_apo_cgd02.log
CFG=config_train_voxbind_frozenenc_channelvit_atomblob7_v2p1
EXP=voxbind_frozenenc_atomblob7_v2p1_sig0.8; EXPDIR=$VOX/exps/$EXP
LOCK=$LOG/chain_frozenenc_sig0.8.lock
...
exec 9>"$LOCK"; flock -n 9 || { log "another frozenenc-sig0.8 chain running — exit"; exit 1; }
...
# 1) wait for the apo-cgd0.2 frozen-enc run to finish (it runs before sig0.8)
DEADLINE=$(( $(date +%s)+12*24*3600 ))
log "waiting for apo-cgd0.2 frozen-enc run to finish (APO-CGD02 DONE) ..."
until grep -q "APO-CGD02 DONE" "$POSTLOG" 2>/dev/null; do
  [ "$(date +%s)" -gt "$DEADLINE" ] && { log "ABORT: timeout waiting for apo-cgd02 run"; exit 1; }
  sleep 600
done
log "apo-cgd0.2 run done. Draining GPUs."
drain
...
log "===== TRAIN sig=0.8 frozen-enc (350 ep, from scratch) exp=$EXP → $RUNLOG ====="
CUDA_VISIBLE_DEVICES=0,1,2,3 "$B/torchrun" --standalone --nproc_per_node=4 "$VOX/train_ddp.py" \
  --config-name="$CFG" smooth_sigma=0.8 wjs.n_targets=0 num_epochs=350 \
  exp_name="$EXP" output_dir="$EXPDIR" hydra.run.dir="$EXPDIR" > "$RUNLOG" 2>&1
log "sig0.8 frozen-enc train exit=$? (target ep350)"
drain
...
log "===== EVAL sig0.8 frozen-enc (@ ep$EP) ====="
bash "$VOX/scripts/frozenenc_full_eval.sh" "$EXPDIR" "$EXPDIR/samples/full_eval_ep${EP}" 100
log "FROZENENC-SIG0.8 CHAIN DONE (@ ep$EP). Add its row to voxbind_results.html (rerun the generator with the new json)."
