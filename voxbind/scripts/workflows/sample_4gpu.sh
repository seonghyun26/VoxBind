...
#
# ── Usage ─────────────────────────────────────────────────────────────────────
#   bash scripts/workflows/sample_4gpu.sh --exp EXP [options]
#
# Required:
...
#   --gpus G        four GPU ids as comma list/range  (inherits queue, else 0-3)
#   --dry-run       print commands and exit
#
# ── Example ───────────────────────────────────────────────────────────────────
#   bash scripts/workflows/sample_4gpu.sh --exp exp_sig0.9 --split test --targets 100
set -euo pipefail
...
VOXBIND_CALLER="sample_4gpu.sh"
VOX="$(voxbind_repo_root)"
PY_DIR="${VOXBIND_PY:-/home/shpark/.conda/envs/voxbind/bin}"
PY="$PY_DIR/python"
LOG=$VOX/log
...
# ── Defaults ──────────────────────────────────────────────────────────────────
EXP=""; SPLIT=test; TARGETS=100; SAMPLES=10; OUT=""
GPUS="${VOXBIND_GPUS:-${CUDA_VISIBLE_DEVICES:-0,1,2,3}}"; DRYRUN=0
...
EXPANDED_GPUS="$(voxbind_expand_gpus "$GPUS")" ||
    die "invalid --gpus specification '$GPUS'"
IFS=',' read -ra GPU_LIST <<< "$EXPANDED_GPUS"
