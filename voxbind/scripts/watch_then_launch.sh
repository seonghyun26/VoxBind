#!/bin/bash
# watch_then_launch.sh — wait for a training checkpoint to land, then run the
...
#
# ── Usage ─────────────────────────────────────────────────────────────────────
#   bash scripts/watch_then_launch.sh [options] -- COMMAND [ARGS...]
#
# Options:
...
#   --log FILE        watcher log (default log/watch_then_launch.log)
...
#   bash scripts/watch_then_launch.sh \
...
#   bash scripts/watch_then_launch.sh --wait exps/RUN/checkpoint_e0099.pth.tar --watch RUN \
#       --expect dataset/data/xray_crops_aligned_v2/stats.json \
#       -- bash -c 'python dataset/00e_v2_pool_norm_crossdocked.py && \
...
LOG=$VOX/log
ts(){ date "+%Y-%m-%d %H:%M:%S"; }
die(){ echo "watch_then_launch.sh: $*" >&2; exit 2; }
...
[[ -z "$WATCH_LOG" ]] && WATCH_LOG=$LOG/watch_then_launch.log
mkdir -p "$LOG"
...
proc_alive(){
    ...
    while read -r pid; do
        ...
        case "$cmd" in *watch_then_launch.sh*) continue;; esac
        return 0
