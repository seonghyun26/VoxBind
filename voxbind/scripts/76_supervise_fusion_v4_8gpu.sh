#!/usr/bin/env bash
# 76_supervise_fusion_v4_8gpu.sh
#   Crash-resume supervisor around 75_train_fusion_v4_cdgv2_8gpu.sh.
#
#   Same rationale as 71_supervise_voxbind_base_8gpu.sh: svr12 is shared, and the
#   PoseCheck/Vina evaluations under ~/prj-denovo/baselines have fork-bombed it to
#   load-avg 22,730 and made the kernel OOM-kill all 8 training ranks at once. A bare
#   torchrun launch just dies; this relaunches from the last checkpoint once the box
#   has settled and the GPUs are free again.
#
#   TOTAL_EPOCHS is the ABSOLUTE target (epoch indices 0..TOTAL_EPOCHS-1). train_ddp.py's
#   loop is range(start_epoch, start_epoch + num_epochs) and load_checkpoint returns the
#   checkpoint's own epoch, so a resume needs num_epochs = TOTAL_EPOCHS - ckpt_epoch to
#   land on the same final epoch.
#
...
for (( attempt=0; attempt<=MAX_RETRIES; attempt++ )); do
    ...
    if [ "$attempt" -eq 0 ] && [ "$e" -lt 0 ]; then
        log "attempt $attempt: fresh start (warm-started from the vanilla e349), $TOTAL_EPOCHS epochs"
        EXP_NAME="$EXP_NAME" NUM_EPOCHS="$TOTAL_EPOCHS" \
            bash scripts/75_train_fusion_v4_cdgv2_8gpu.sh
        rc=$?
