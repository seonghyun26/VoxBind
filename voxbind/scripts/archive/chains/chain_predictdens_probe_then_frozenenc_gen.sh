...
#   setsid nohup bash scripts/archive/chains/chain_predictdens_probe_then_frozenenc_gen.sh \
...
resume_denoiser(){ [ -f "$DENOISER_CK" ] && { setsid nohup bash "$VOX/scripts/archive/chains/chain_sig1.0_keepgoing.sh" > "$LOG/chain_sig1.0_keepgoing.log" 2>&1 & log "denoiser resume re-armed."; } || log "WARN: denoiser ckpt missing ($DENOISER_CK) — cannot resume"; }
