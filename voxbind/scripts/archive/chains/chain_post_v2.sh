...
#   setsid nohup bash scripts/archive/chains/chain_post_v2.sh > log/chain_post_v2.log 2>&1 &
set -uo pipefail
...
bash "$VOX/scripts/archive/workflows/vanilla_full_eval.sh" "$EP923" "$EP923/samples/full_eval_ep923" 100

# ── 3) EVAL v2 (dedicated) ───────────────────────────────────────────────────
log "===== STAGE 3: EVAL v2 (vanilla sig0.9 @ ep$V2EP) ====="
bash "$VOX/scripts/archive/workflows/vanilla_full_eval.sh" "$VOX/exps/$V2" "$VOX/exps/$V2/samples/full_eval_ep${V2EP}" 100
...
bash "$VOX/scripts/archive/workflows/vanilla_full_eval.sh" "$SIG10_DIR" "$SIG10_DIR/samples/full_eval_ep${S10EP}" 100
