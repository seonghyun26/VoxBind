...
#     setsid nohup bash scripts/archive/chains/chain_plinder_6case_then_resume.sh \
...
LOCK=$LOG/chain_plinder_coordsdens.lock

# 6-case grid: config-name -> exp-name (exp-name also = config's exp_name)
# coords+density (atomblob_density, 12ch) x {ViT, ChannelViT}, run one by one:
CONFIGS=(
  config_train_atomblob_density_vit_mae_40m_plinder
  config_train_atomblob_density_channelvit_40m_plinder
)
