...
#   setsid nohup bash scripts/archive/chains/chain_frozenenc_restart.sh > log/chain_frozenenc_restart.log 2>&1 &
set -uo pipefail
...
resume_denoiser(){ [ -f "$DENOISER_CK" ] && { setsid nohup bash "$VOX/scripts/archive/chains/chain_sig1.0_keepgoing.sh" > "$LOG/chain_sig1.0_keepgoing.log" 2>&1 & log "denoiser resume re-armed."; } || log "WARN: denoiser ckpt missing — cannot resume"; }
