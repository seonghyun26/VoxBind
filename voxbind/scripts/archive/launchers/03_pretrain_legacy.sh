...
#                   May be omitted when VOXBIND_GPUS or CUDA_VISIBLE_DEVICES is
...
# ── Defaults ──────────────────────────────────────────────────────────────────
MODE=""; GPUS="${VOXBIND_GPUS:-${CUDA_VISIBLE_DEVICES:-}}"; DATA_VER="v1"; WEIGHTED=0; GRADMAG=0
...
[[ -n "$GPUS" ]] || die "--gpus is required unless inherited from scripts/99_chain.sh"

# ── GPU spec → CUDA list + nproc ──────────────────────────────────────────────
CUDA_LIST="$(voxbind_expand_gpus "$GPUS")" || die "could not parse --gpus '$GPUS'"
...
if [[ -n "$EXPERIMENT" ]]; then
    ...
    RC=$?
    echo "[$(ts)] $EXP done (exit $RC)  ->  exps/$EXP"
    exit "$RC"
fi
