#!/bin/bash
# 260630_ar_cvit_radembed_k8_dist.sh — DRAMATIC change #7: LEARNABLE RADIUS-CONDITIONED ATOM EMBEDDING.
...
# Because φ sees the continuous radius and k=8<7, it's non-absorbable (a learned soft vocabulary) and
# recovers element separation that roleblob's size-encoding sums away. Caveat: lig S and P share vdW
# 1.8Å so radius cannot separate them (by construction). k=8. [7,4,2]-raw base; bsz8×accum4×4gpu =
...
GPUS=0,1,2,3; PORT=29619; RID=arRadDist
EXP=260630_ar_cvit_radembed_k8_dist

echo "===== AR radius-embed k=8 START $(date '+%m-%d %H:%M:%S') GPU=$GPUS ====="
CUDA_VISIBLE_DEVICES=$GPUS "$PY/torchrun" --nnodes=1 --nproc_per_node=4 \
    ...
    model.channel_groups=[7,4,2] +model.radius_embed_k=8 "+model.radius_embed_scales=[0.0,2.0,4.0]" \
...
echo "===== AR radius-embed k=8 COMPLETE $(date '+%m-%d %H:%M:%S') ====="
