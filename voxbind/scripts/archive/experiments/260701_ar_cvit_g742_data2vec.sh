#!/bin/bash
# 260701_ar_cvit_g742_data2vec.sh — CV/NLP trick B: data2vec latent-target MAE (Meta, modality-agnostic).
# Controlled change off the ChannelViT [7,4,2] winner (ρ 0.637): the ONLY thing added is
# +mae.data2vec_weight=0.1 — alongside MAE raw-voxel reconstruction, the student also predicts the
# EMA-teacher's contextualized per-patch LATENT (LayerNorm'd) at masked patches (smooth-L1) — a semantic
# target instead of low-level density texture. Predictor head is pretrain-only (not in encoder.*) →
# frozen probe bit-exact. Opt-in, original code bit-identical at weight=0. GPU 0-3, bsz8×accum4 eff128, compile OFF.
set -uf
...
GPUS=0,1,2,3; PORT=29623; RID=arG742D2V
EXP=260701_ar_cvit_g742_data2vec

echo "===== AR [7,4,2] DATA2VEC=0.1 (MAE + latent-distill) START $(date '+%m-%d %H:%M:%S') GPU=$GPUS ====="
CUDA_VISIBLE_DEVICES=$GPUS "$PY/torchrun" --nnodes=1 --nproc_per_node=4 \
    ...
    model.channel_groups=[7,4,2] +mae.data2vec_weight=0.1 \
