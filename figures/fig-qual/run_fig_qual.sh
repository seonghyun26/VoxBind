...
#
# Usage:
#   bash figures/fig-qual/run_fig_qual.sh
#   PORT=8877 HOST=0.0.0.0 bash figures/fig-qual/run_fig_qual.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP="${SCRIPT_DIR}/fig_qual_server.py"
# Default is THIS box's env; VOXBIND_PYTHON overrides it. It used to name a path on
# another machine, which failed with "VoxBind Python not found" and no way to guess why.
VOXBIND_PYTHON="${VOXBIND_PYTHON:-/opt/conda/envs/voxbind/bin/python}"
HOST="${HOST:-127.0.0.1}"
PORT="${PORT:-8766}"
