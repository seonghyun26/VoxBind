...
Writes into its own directory (the previous baseline-only figures were copied to
...
into the y-axis name, the size bin has nowhere else to live inside the frame.
  * posecheck_all_by_atom_range.json / .csv
    ...
    /opt/conda/envs/voxbind/bin/python \
        notebook/html/260910/fig-posecheck/build_posecheck_all_by_atom_range.py
"""
import csv
import glob
import json
...
OUT = HERE                       # the builder lives beside the figures it writes
