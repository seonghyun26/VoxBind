...
data in one variant and not another is worse than one that is simply never in the way.
    ...
    /opt/conda/envs/voxbind/bin/python \
        notebook/html/260910/fig-ref-ligand-similarity/build_similarity_figure.py
"""
import csv
import os
import sys
...
CSV = os.path.join(HERE, "reference_similarity.csv")   # written by the sibling builder
OUT = HERE
...
def load():
    ...
    with open(CSV, encoding="utf-8") as handle:
        rows = [{"method": r["method"],
                 ...
                 "novelty": num(r.get("novelty")),
                 "scaffold_novelty": num(r.get("scaffold_novelty")),
                 "snn": num(r.get("snn"))}
...
def variant_dumbbell(rows):
    ...
    """
    # Sized so the PLOT AREA is 0.8x what the 8.67 in draft had, with the type untouched.
    # Shrinking the figure by 0.8 would not do that: the furniture outside the axes -- row
    # names, tick labels, the gutter -- is type, so it keeps its physical size, and the
    # plot area would absorb the whole cut and land near 0.73x. Measured at 8.67 in the
    # two panels were 4.115 + 2.352 = 6.467 in of plot area against 2.203 in of furniture,
    # so 8.67 - 0.2 * 6.467 leaves the furniture alone and takes the fifth off the axes.
    ...
    for i, row in enumerate(rows):
        ...
        if row["novelty"] is None:
            for axis, at in ((ax, 0.5), (ax2, 0.5)):
                axis.text(at, i, "not measured here", transform=axis.get_yaxis_transform(),
                          ha="center", va="center", fontsize=11, color=BASE_COLOR,
                          style="italic")
            continue
        ...
        ax.plot(lo, i, marker="o", markersize=7.0, color=c, mfc="white",
                markeredgewidth=2.7, markeredgecolor=c, zorder=3)
        ax.plot(hi, i, marker="o", markersize=10.2, color=c, markeredgewidth=0, zorder=4)
