"""build_strain_by_size_curve.py — Figure 7 of results_drug_design.html.
...
The statistic is the MEDIAN, not the mean.  Section 2.1 already says why: strain is
heavy-tailed and a handful of relaxation failures per model drag the mean by orders of
magnitude (TargetDiff's pooled mean is ~4e7 against a median of 361).  The y axis is
...
OUT = os.path.join(HERE, "strain_by_size_curve")
...
MODELS = [
    ("TargetDiff", "targetdiff", "/home1/irteam/base_drug/eval/targetdiff", "#3498db", 1.9),
    ("VoxBind σ=0.9", "vanilla", f"{E}/_vanilla_ep923/samples/full_eval_ep923", "#1abc9c", 2.4),
    ("Ours", "ours", f"{E}/voxbind_frozenenc_atomblob7_v2p1_sig0.9/samples/full_eval_ep350", "#2ecc71", 3.0),
    ("Ours · v2", "ours_v2", f"{E}/samples_reference_receptor_ed_ep350", "#7fcf9d", 1.9),
]
REF_COLOR = "#8e9baa"
...
for name, key, _, col, lw in MODELS:
    ...
    if not xs:
        continue
    med = [st.median(d[a]) for a in xs]
    band = np.array([
        np.percentile(
            np.median(rng.choice(np.asarray(d[a]), (BOOT, len(d[a])), replace=True), axis=1),
            [2.5, 97.5])
        for a in xs])
...
ry = [st.median(REF[a]) for a in rx]
...
ax.set_title("PoseCheck strain energy at every ligand size", fontsize=17, loc="left", pad=30)
ax.text(0, 1.012, "78 CrossDocked pockets · median per exact heavy-atom count · lower is better",
...
fig.text(0.5, 0.030,
         ...
         f"bands are 95% bootstrap CIs of the median ({BOOT:,} resamples, seed {SEED})",
...
for name, _, _, _, _ in MODELS:
    ...
    summary[name] = {
        "sizes": xs,
        "median_strain": [round(st.median(d[a]), 3) for a in xs],
        "n_molecules": [len(d[a]) for a in xs],
    }
summary["Reference ligand"] = {
    "sizes": rx,
    "median_strain": [round(st.median(REF[a]), 3) for a in rx],
