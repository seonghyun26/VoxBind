...
EXP = [
  ("run", "260818_cdg_100m_v24_g742_pergroup_m075_ema09999", "generalize",
   ...
   "<b>resumed on GPU 4-7</b> @ ep33/50 — was launched on GPU 2-7 which <b>spans both NUMA nodes</b> (0-3 vs 4-7, only PCIe/UPI cross-link → allreduce straggler = 2350 s/ep, 2× slow); killed at ep32, resumed from ckpt on a <b>single NUMA island (4-7)</b> → ~1090 s/ep, ETA ~5h. Probe (full+CL3) when done", "na"),
  ("q", "260819_cdg_100m_pergroup_lr_cosine", "schedule",
   "<b>Cosine LR schedule vs constant</b>: EVERY run in this campaign trains at <b>constant lr=1e-4</b> (default <code>lr_schedule=constant</code>) — unusual for MAE/SSL, where cosine-decay + linear warmup is the standard recipe (lets the encoder settle into a sharper minimum in the final epochs). Opt-in knobs already exist (<code>lr_schedule=cosine</code>, <code>lr_warmup_epochs</code>, <code>lr_min</code>). Re-run the per_group [7,4,2] champion recipe with cosine (warmup ~3ep → 1e-4, decay → ~0) — is the constant-LR plateau partly a schedule artifact? Prediction: small effect (±0.01); the plateau has looked data/capacity-bound, not recipe-bound.",
   "queued — launch on GPU 4-7 after run1 finishes (~5h)", "na"),
