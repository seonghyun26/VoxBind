...
class QualitativeGrid:
    def __init__(self, catalog, style, methods, targets, panel_px=310, pocket_extent=8.0,
        ...
        for row_index, target in enumerate(targets):
            ...
            target_menu = widgets.Dropdown(options=[(catalog.reference(t)["label"], t) for t in common],
    ...
    def _load_row(self, row_index):
        ...
        for method in self.methods:
            records, invalid = self.catalog.records(method, target)
            state["records"][method] = records
            saved = self.selections.get(target, {}).get(method)
            value = 0
            if saved is not None:
                matches = [i for i, r in enumerate(records)
                           if str(r["path"]) == saved["path"] and r["index"] == saved["record_index"]]
                if not matches:
                    raise ValueError(f"Saved selection is unavailable: {target}, {method}")
                value = matches[0]
            menu = widgets.Dropdown(options=[(r["label"], i) for i, r in enumerate(records)], value=value,
                                    layout=widgets.Layout(width=f"{self.panel_px - 12}px"))
            state["menus"][method] = menu
            controls.append(widgets.VBox([
                widgets.HTML(f"<b>{html.escape(method)}</b> · {len(records)} ligands · {len(invalid)} skipped"), menu,
            ]))
            menu.observe(lambda change, row=row_index: self._refresh(row), names="value")
        ...
        for state in self.rows:
            ...
            for method, menu in state["menus"].items():
                ...
                rows.append(dict(target=state["target"], method=method,
                                 ...
                                 heavy_atoms=len(xyz), centroid_offset_A=round(offset, 2),
    ...
    def manifest(self):
        return dict(methods=self.methods, targets=[s["target"] for s in self.rows],
                    ...
                    style_source="fig1.ipynb", ligand_color=self.style["LIGAND_COLOR"],
